...
class MongoClient {
	...
	_connectionInfoToMongoUrl(connection) {
		var connectionStringParams = this._connectionStringToParameterMap(connection.connectionString)
		...
		var url = `mongodb://${user}:${password}@${hostname}:27017/${database}`;
		return url;
	}

	_connectionStringToParameterMap(connectionString) {
		var paramEntriesArray = connectionString.split(";");
		var result = paramEntriesArray.reduce(function(map, paramEntry) {

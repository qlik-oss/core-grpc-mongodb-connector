...
session.open()
  ...
  .then((tableData) => {
    ...
    const firstRow = tableData.find(row => row.qValue[1].qNumber === 5871);
    if (!firstRow) {
      return Promise.reject(new Error('Could not find row with id 5871'));
    }

    // Check if the contents of that row matches the expected.
    let firstRowAsString = firstRow.qValue.map(obj => obj.qText).reduce((a, b) => `${a}:${b}`);

    // Removing the first field value assuming it's the _id generated by mongodb
    firstRowAsString = firstRowAsString.substr(firstRowAsString.indexOf(':') + 1);
